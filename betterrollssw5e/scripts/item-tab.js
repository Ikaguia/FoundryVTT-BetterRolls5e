...
export async function addBetterRollsContent(app, protoHtml) {
	...
	const betterRollsTabString = `<a class="item" data-group="primary" data-tab="betterRollssw5e">${i18n("Better Rolls")}</a>`;
	tabSelector.append($(betterRollsTabString));

	const settingsContainer = html.find(".sheet-body");
	const altSecondaryEnabled = game.settings.get("betterrollssw5e", "altSecondaryEnabled");
	...
	const betterRollsTemplate = await renderTemplate("modules/betterrollssw5e/templates/red-item-options.html", {
		SW5E: CONFIG.SW5E,
		item,
		canConsume,
		hasQuantity,
		...
		damageTypes: CONFIG.betterRollssw5e.combinedDamageTypes,
		altSecondaryEnabled,
	...
	if (activate) {
		app._tabs[0].activate("betterRollssw5e");
		app.setPosition();
		activate = false;
	}
	...
	if (game.settings.get("betterrollssw5e", "damageContextPlacement") !== "0") {
		...
		const placeholder = game.settings.get("betterrollssw5e", "contextReplacesDamage") ? "br5e.settings.label" : "br5e.settings.context";

		damageRolls.forEach((damageRoll, i) => {
			const contextField = $(`<input type="text" name="flags.betterRollssw5e.quickDamage.context.${i}" value="${(item.data.flags.betterRollssw5e.quickDamage.context[i] || "")}" placeholder="${i18n(placeholder)}" data-dtype="String" style="margin-left:5px;">`);
			...
			$($($(damageRoll)[0].parentElement).find(`a.delete-damage`)).click(async _ => {
				const contextFlags = Object.values(item.data.flags.betterRollssw5e.quickDamage.context);
				contextFlags.splice(i, 1);
				item.update({
					[`flags.betterRollssw5e.quickDamage.context`]: contextFlags,
				});
			});
		});
		...
		if (getProperty(item, "data.flags.betterRollssw5e.quickOther")) {
			...
			const otherContextField = $(`<input type="text" name="flags.betterRollssw5e.quickOther.context" value="${(item.data.flags.betterRollssw5e.quickOther.context || "")}" placeholder="${i18n(placeholder)}" data-dtype="String" style="margin-left:5px;">`);
